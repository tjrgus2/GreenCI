...
그 이력의 중앙값과 MAD를 기준으로 현재 실행을 비교합니다. 코드를 체크아웃하지 않고, 읽어온
GitHub API 외에는 어디로도 아무것도 보내지 않습니다.
...
되고, 아무 의미 없는 숫자가 그럴듯한 모습으로 나옵니다.
...
- GreenCI 서버·데이터베이스·계정이 없습니다. 유일한 외부 통신 대상은 분석 대상 저장소의 GitHub
  API입니다.
- 텔레메트리, 애널리틱스, LLM 호출, 소스 업로드가 없습니다.
- 분석기는 Pull Request 코드를 체크아웃하거나 실행하지 않고, 저장소가 제어하는 문자열을 셸에
  넘기지 않으며, `eval`을 쓰지 않습니다.
...
- 실패 로그 파싱은 기본 비활성입니다. 켜면 크기 제한과 자격 증명 마스킹이 적용되고 메모리에서만
  처리하며 디스크에 쓰지 않습니다.
